...
#define TERRAIN_DRAW_RADIUS 8
#define TERRAIN_CREATE_RADIUS 9

// moved the enum biome to the terrain generator class
...
private:
    ...
    std::unordered_set<int64_t> newChunks;

    // chunks with block data to be sent for VBO creation
    std::unordered_map<int64_t, Chunk*> chunksWithBlockData;
    std::mutex chunksWithBlockDataMutex;

    std::unordered_map<int64_t, Chunk*> chunksWithVBOData;
    std::mutex chunksWithVBODataMutex;

    // use to check if block data has been generated and hence a given the memory of surrounding block can be cleared
    std::unordered_map<int64_t, bool> chunksWithVBOGenerated;
...
public:
    ...
    void draw(glm::vec2 playerPos, ShaderProgram *shaderProgram);


    // Initializes the Chunks that store the 64 x 256 x 64 block scene you
    // see when the base code is run.
    void CreateTestScene();
    bool terrainExpansion(glm::vec3 playerPos);

    void ProceduralHelp(Chunk* currChunk);
    ...
    void blockWorker(Chunk *);
    void spawnVBOWorkers();
    void VBOWorker(Chunk *);

    bool hasTerrainGenerationZoneAt(glm::ivec2 zone);
    BlockType genBlocktype(glm::ivec3 pos, Biome biome);

    void genSparseFeatures(glm::ivec3 pos, Biome biome);
    void genContFeatures(glm::ivec3 pos, Biome biome);
};
